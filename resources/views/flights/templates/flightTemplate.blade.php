<div class="card mt-2">
    <div class="card-header  ">
        <div class="row">
            <div class="col-md-6">
                <h3 class="card-title  float-left"> {{ $flight->getRequester()->name}}
                    @if($flight->permission)
                    Ref:: {{ $flight->permission->ref}}
                    @endif
                </h3>
            </div>
            <div class="col-md-6">
                <span class=" float-right"> {{$flight->callsign}} <span class="badge p-2 @if($flight->state_id ==2) badge-warning @elseif($flight->state_id ==1)  badge-success  @elseif($flight->state_id ==3 ) bg-danger text-white @else($flight->state_id ==4 ) badge-danger text-white @endif  "> {{$flight->state->name}}</span></span>
            </div>
        </div>
    </div>
    <div class="card-body">
            ...
            <table class="table table-hover">
                <tr class="border">
                    ...
                    <td class="float-right">
                        ...
                        {{$flight->submission->ref}} <small> ({{$flight->submission->created_at->diffForHumans()}}) </small>
                        @else
